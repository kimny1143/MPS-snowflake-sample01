...
このプロジェクトは **Snowflake のキャッチアップを目的とした技術検証プロジェクト**です。

[MUED（ミュード）](https://www.mued.jp)（[GitHub](https://github.com/kimny1143/mued_lms_fgm)）は音楽教育のためのプラットフォームですが、その中の「AI による教材生成」や「RSS フィードによる教材管理」といった機能を **Snowflake のデータプラットフォーム上で実装した場合**を想定して開発されています。

### 現在の状況と今後の展望

- **現在**: Snowflake アカウントプランの制約により、シンプルなテキストベースの検索機能のみを実装
- **将来**: Snowflake Cortex が利用可能になった際には、以下の高度な機能が実現可能
  - セマンティック検索（意味的な類似性による検索）
  - AI による自動要約生成
  - ベクトル埋め込みによる高精度な推薦システム

従来の OpenAI API を使った実装から、Snowflake Cortex への移行により、外部 API キーの管理が不要になり、Snowflake 内で完結したデータパイプラインを構築できます。
